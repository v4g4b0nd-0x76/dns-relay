...
test("first launch installs and starts through one setup action", async ({
  page,
}) => {
...
test("first launch explains the setup without dead navigation", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("Confirm locations", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText("Choose resolver", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText("Install service", { exact: true }),
  ).toBeVisible();
});

test("cancelled elevation leaves first launch recoverable", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
  ...
  await expect(
    page.getByRole("button", { name: "Install and start" }),
  ).toBeEnabled();
});

test("existing install is protected until explicit adoption", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
  await page.goto(`${production}/?fixture=existing`);
  await expect(
    page.getByRole("heading", { name: "Adopt DNS Relay" }),
  ).toBeVisible();
  await expect(
    page.getByText("protected from default replacement"),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Adopt existing configuration" })
    .click();
  await expect(page.locator("[data-view='dashboard']")).toBeVisible();
...
test("partial install exposes a safe repair path", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Repair DNS Relay" }),
  ).toBeVisible();
...
test("repair preserves an existing config behind adoption", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
...
test("stopped service keeps non-connection telemetry errors visible", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
...
test("service state suppresses connection-refused endpoint noise", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
...
test("restart failure is reported without a false success state", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
...
test("initial service status failure keeps independent dashboard data", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
...
test("polling leaves an active modal and its focus intact", async ({
  page,
}) => {
  await openApp(page);
  ...
  await expect(page.getByLabel("Domain pattern")).toHaveValue(
    "editing.example",
  );
...
test("desktop shell uses the available workspace without stretching indefinitely", async ({
  page,
}) => {
...
for (const width of [390, 420, 760, 1024, 1440]) {
  test(`production pages have no horizontal overflow at ${width}px`, async ({
    page,
  }) => {
...
test("dashboard fits the default desktop height without initial scrolling", async ({
  page,
}) => {
...
test("dashboard service summary stays compact at both layouts", async ({
  page,
}) => {
  for (const width of [420, 1024]) {
    await openApp(page, width, 720);
    const hero = await page
      .locator("[data-view='dashboard'] .hero")
      .boundingBox();
...
test("boolean config fields render as app switches", async ({ page }) => {
  ...
  const checkboxCount = await page
    .locator(".check-row input[type='checkbox']")
    .count();
  await expect(page.locator(".check-row .switch")).toHaveCount(checkboxCount);
});

test("compact dashboard keeps service facts readable in two columns", async ({
  page,
}) => {
  await openApp(page);

  const facts = await page.locator(".detail-list > div").evaluateAll((items) =>
    items.map((item) => ({
  ...
  expect(facts.every(({ clipped, width }) => !clipped && width >= 85)).toBe(
    true,
  );
});
...
test("desktop forms use the compact screenshot spacing", async ({ page }) => {
  ...
  const spacing = await page
    .locator("[data-view='relay'] .relay-settings")
    .evaluate((card) => {
...
test("relay settings separate switches from text fields on desktop", async ({
  page,
}) => {
  await openApp(page, 1024, 768);
  await page.locator("[data-target='relay']").click();
...
test("activity data regions use desktop width and stack compactly", async ({
  page,
}) => {
...
test("compact dashboard keeps flat cards and the horizontal service summary", async ({
  page,
}) => {
  await openApp(page);
...
test("file action buttons do not stretch across toolbars", async ({ page }) => {
  ...
  const importButton = await page
    .getByText("Import blocklist", { exact: true })
    .boundingBox();
...
test("empty operational views are actionable", async ({ page }) => {
  ...
  await page.locator("[data-target='relay']").click();
  await expect(
    page.getByText("No relay endpoints", { exact: true }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "Add relay" })).toBeVisible();
});

test("compact resolver controls leave the address readable", async ({
  page,
}) => {
...
test("empty rules keeps one primary add action inside the empty state", async ({
  page,
}) => {
  ...
  await expect(
    page.locator(".empty-state").getByRole("button", { name: "Add rule" }),
  ).toBeVisible();
});

test("compact relay action stays above the bottom navigation", async ({
  page,
}) => {
  await openApp(page, 420, 690);
  await page.locator("[data-target='relay']").click();
  ...
  const action = await page
    .getByRole("button", { name: "Add relay" })
    .boundingBox();
...
test("compact relay visible content stays above the bottom navigation", async ({
  page,
}) => {
  await openApp(page, 420, 720);
  await page.locator("[data-target='relay']").click();
  ...
  const layout = await page.evaluate(() => {
    ...
    const last = document
      .querySelector("[data-view='relay'] > :last-child")
...
test("compact pending changes stay clear of scrolled settings", async ({
  page,
}) => {
  await openApp(page, 420, 690);
  await page.locator("[data-target='settings']").click();
  ...
  await page.locator("[data-view-host]").evaluate((view) => {
    view.scrollTop = view.scrollHeight;
  });

  const gap = await page.evaluate(() => {
    const lastCard = document
      .querySelector("[data-view='settings'] .card:last-of-type")
      .getBoundingClientRect();
    const dirtyBar = document
      .querySelector("[data-dirty-bar]")
      .getBoundingClientRect();
...
test("dashboard activity preview shows recent activity", async ({ page }) => {
  ...
  await expect(page.locator("[data-dashboard-activity]")).toContainText(
    "blocked tracker.example",
  );
  await expect(page.locator("[data-dashboard-activity]")).not.toContainText(
    "No recent events",
  );
});

test("dashboard metric row stays separated from activity preview", async ({
  page,
}) => {
  await openApp(page, 1440);
  ...
  const gap = await page.evaluate(() => {
    ...
    const preview = document
      .querySelector("[data-dashboard-activity]")
      .getBoundingClientRect();
...
test("settings listener field stays aligned in the field column", async ({
  page,
}) => {
  await openApp(page, 1440);
  await page.locator("[data-target='settings']").click();
  ...
  const interval = await page
    .getByLabel("Hot reload interval (ms)")
    .boundingBox();
...
test("resolver discovery keeps switches and text fields aligned", async ({
  page,
}) => {
  await openApp(page, 1440);
  await page.locator("[data-target='resolvers']").click();
  ...
  await expect(
    page.locator("[data-resolver-options] .option-column .check-row"),
  ).toHaveCount(3);
  await expect(
    page.locator(
      "[data-resolver-options] .field-column input, [data-resolver-options] .field-column textarea",
    ),
  ).toHaveCount(3);
});
...
test("resolver discovery is a labeled aligned section", async ({ page }) => {
  ...
  await page.locator("[data-target='resolvers']").click();
  await expect(
    page.locator("[data-resolver-options] .section-label > strong"),
  ).toHaveText("Discovery");
  const columns = await page
    .locator("[data-resolver-options]")
    .evaluate(
      (card) => getComputedStyle(card).gridTemplateColumns.split(" ").length,
    );
  expect(columns).toBe(2);
});

test("settings groups keep switches and text fields aligned", async ({
  page,
}) => {
  await openApp(page, 1440);
  await page.locator("[data-target='settings']").click();
  ...
  await expect(
    page.locator("[data-settings-core] .option-column .check-row"),
  ).toHaveCount(3);
  await expect(
    page.locator("[data-settings-core] .field-column input"),
  ).toHaveCount(2);
  await expect(
    page.locator("[data-settings-metrics] .option-column .check-row"),
  ).toHaveCount(2);
  await expect(
    page.locator(
      "[data-settings-metrics] .field-column input, [data-settings-metrics] .field-column select, [data-settings-metrics] .field-column textarea",
    ),
  ).toHaveCount(4);
});

test("settings hides developer fixtures and collapses Raw TOML", async ({
  page,
}) => {
  await openApp(page, 1024, 768);
  await page.locator("[data-target='settings']").click();
...
test("settings service actions share one bounded row", async ({ page }) => {
  ...
  const layout = await page
    .locator("[data-settings-service]")
    .evaluate((card) => {
      const title = card
        .querySelector(".section-heading > div")
        .getBoundingClientRect();
      const actions = card
      ...
      return {
        ...
        rowGap: Math.abs(
          actions.top + actions.height / 2 - (title.top + title.height / 2),
        ),
...
test("settings lower utility cards use compact desktop spacing", async ({
  page,
}) => {
  ...
  const layout = await page.evaluate(() => {
    const obfs = document
      .querySelector("[data-settings-obfs]")
      .getBoundingClientRect();
    const service = document
      .querySelector("[data-settings-service]")
      .getBoundingClientRect();
    const advanced = document
      .querySelector("[data-settings-advanced]")
      .getBoundingClientRect();
    const keyLabel = document
      .querySelector("[data-settings-obfs] .key-tools .muted")
      .getBoundingClientRect();
    const keyButton = document
      .querySelector("[data-action='generate-obfs-secret']")
      .getBoundingClientRect();
    return {
...
test("settings metrics history fields avoid a blank left column", async ({
  page,
}) => {
  await openApp(page, 1440, 900);
  await page.locator("[data-target='settings']").click();
  ...
  await expect(
    page.locator("[data-settings-metrics] .history-fields"),
  ).toBeVisible();
  const layout = await page.evaluate(() => {
    const card = document
      .querySelector("[data-settings-metrics]")
      .getBoundingClientRect();
    const history = document
      .querySelector("[data-settings-metrics] .history-fields")
      .getBoundingClientRect();
    const options = document
      .querySelector("[data-settings-metrics] .option-column")
      .getBoundingClientRect();
    const fields = document
      .querySelector("[data-settings-metrics] .metrics-fields")
      .getBoundingClientRect();
    return {
  ...
  expect(layout.spansMostOfCard).toBeGreaterThan(0.95);
...
test("settings action buttons do not crowd their fields", async ({ page }) => {
  ...
  const layout = await page.evaluate(() => {
    const raw = document
      .querySelector("[data-settings-advanced] textarea")
      .getBoundingClientRect();
    const rawButtons = document
      .querySelector("[data-settings-advanced] .config-actions")
      .getBoundingClientRect();
    return {
...
test("failed service can be stopped from the power control", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
  await page.goto(`${production}/?fixture=service-error`);
...
test("production dialog restores focus and closes with Escape", async ({
  page,
}) => {
  await openApp(page);
  await page.locator("[data-target='rules']").click();
...
test("resolver workflow preserves order, probes, and rejects insecure empty state", async ({
  page,
}) => {
  await openApp(page);
  await page.locator("[data-target='resolvers']").click();
  ...
  await expect(
    page.locator(".toolbar").getByLabel("Secure resolvers only"),
  ).toBeVisible();
  await page
    .getByLabel("New resolver transport")
    .selectOption("quic://9.9.9.9:853");
...
test("rule workflow validates, creates, edits, deletes, and imports", async ({
  page,
}) => {
  await openApp(page);
  await page.locator("[data-target='rules']").click();
  ...
  await expect(page.locator("[data-rule-error]")).toContainText(
    "valid domain and IPv4",
  );
  ...
  await expect(page.getByText("tracker.example", { exact: true })).toHaveCount(
    0,
  );
  await page
    .locator("[data-blocklist-import]")
    .setInputFiles({
      name: "block.txt",
      mimeType: "text/plain",
      buffer: Buffer.from("one.example\n# ignored\ntwo.example\n"),
    });
...
test("relay secrets stay vaulted and probes report latency", async ({
  page,
}) => {
  await openApp(page);
  await page.locator("[data-target='relay']").click();
  ...
  await expect(
    page.getByText("fixture-secret-1", { exact: true }),
  ).toBeVisible();
...
test("existing relay keys can be stored in the vault", async ({ page }) => {
  ...
  await page
    .getByRole("dialog", { name: "Set relay key" })
    .getByRole("textbox", { name: "Relay key" })
    .fill(key);
...
test("apply locks secret replacement before delayed validation", async ({
  page,
}) => {
  await page.setViewportSize({ width: 420, height: 720 });
  ...
  await page
    ...
    .fill("https://relay-two.example/dns-query");
  await page.keyboard.press("Tab");
  await page.getByRole("button", { name: "Apply changes" }).click();
...
test("first history edit creates a complete optional retention object", async ({
  page,
}) => {
  await openApp(page);
  await page.locator("[data-target='settings']").click();
...
test("activity sources filter, pause, export, and clear independently", async ({
  page,
}) => {
  await openApp(page);
  await page.locator("[data-target='activity']").click();
...
test("raw TOML validation and secret-free export remain explicit", async ({
  page,
}) => {
  await openApp(page);
  await page.locator("[data-target='settings']").click();
...
for (const width of [420, 1024]) {
  test(`production navigation does not overlap at ${width}px`, async ({
    page,
  }) => {
  ...
  test(`settings actions fit their cards at ${width}px`, async ({ page }) => {
    ...
    const overflow = await page
      .locator(
        ".card input, .card select, .card textarea, .card button, .card label.button",
      )
    ...
    await expect(page.locator("[data-config-import]").locator("..")).toHaveCSS(
      "flex-grow",
      "0",
    );
  });
