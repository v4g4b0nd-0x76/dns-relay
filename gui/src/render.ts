...
function renderDashboard(state: ShellState) {
  ...
  return `<section class="view dashboard-view" data-view="dashboard">${heading("Control", "Dashboard", "Service and resolver state at a glance")}
    ...
    <div class="card activity-preview" data-dashboard-activity>${renderActivityPreview(state)}</div>
  </section>`;
}
...
function renderActivityPreview(state: ShellState) {
  const rows = [...(state.logs.value ?? []), ...(state.history.value ?? [])].slice(-3);
  if (!rows.length) {
    return emptyState("activity", "No recent events", "Service logs and query history will appear here once DNS Relay starts.", '<button class="button" data-action="navigate" data-view-target="activity">Open activity</button>');
  }
  return `<div class="activity-snippet"><i data-lucide="activity"></i><div>${rows.map((line) => `<span>${escapeHtml(line)}</span>`).join("")}</div><button class="button" data-action="navigate" data-view-target="activity">Open activity</button></div>`;
}
...
function renderResolvers(state: ShellState) {
  ...
  return `<section class="view" data-view="resolvers">${heading("Upstreams", "Resolvers", "Ordered UDP, DoH, and DoQ endpoints")}
    <div class="toolbar resolver-toolbar"><div class="security-toggle">${checkField("Secure resolvers only", "secure_only", draft?.secure_only, state.applying)}</div><label><span class="sr-only">New resolver transport</span><select aria-label="New resolver transport" data-resolver-template><option value="https://1.1.1.1/dns-query">DoH</option><option value="quic://9.9.9.9:853">DoQ</option><option value="1.1.1.1:53">UDP</option></select></label><button class="button primary" data-action="add-resolver"><i data-lucide="plus"></i> Add resolver</button></div>
...
function renderActivity(state: ShellState) {
  ...
  return `<section class="view" data-view="activity">${heading("Observability", "Activity", "Bounded service logs and query history")}
    ...
    <div class="activity-grid"><section class="card"><div class="section-heading"><p class="eyebrow">Service logs</p><span class="badge">${logs.length}</span></div>${logs.length ? `<ul class="log-list">${logs.map((line) => `<li>${escapeHtml(line)}</li>`).join("")}</ul>` : '<div class="empty-state">No matching logs</div>'}</section><section class="card"><div class="section-heading"><p class="eyebrow">Query history</p><span class="badge">${history.length}</span></div>${history.length ? `<ul class="log-list">${history.map((line) => `<li>${escapeHtml(line)}</li>`).join("")}</ul>` : '<div class="empty-state">No matching history</div>'}</section></div>
  </section>`;
}
